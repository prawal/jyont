...
public class ScreenAV extends BaseScreen {
	...
	protected void onStart() {
		...
		final KeyguardManager keyguardManager = Joynt.getKeyguardManager();
		...
		final PowerManager powerManager = Joynt.getPowerManager();
		...
		if (mProxSensor == null && !Joynt.isBuggyProximitySensor()) {
		...
		if (resultCode == RESULT_OK) {
			...
			case SELECT_CONTENT:
				if (mAVSession != null) {
					...
					// ScreenFileTransferView.sendFile(mAVSession.getRemotePartyUri(),
					// selectedContentPath);
				}
				break;
			}
	...
	@Override
	public boolean back() {
		boolean ret = mScreenService.show(HomeActivity.class);
		if (ret) {
	...
	static class MyProxSensor implements SensorEventListener {
		...
		MyProxSensor(ScreenAV avScreen) {
			mAVScreen = avScreen;
			mSensorManager = Joynt.getSensorManager();
		}
