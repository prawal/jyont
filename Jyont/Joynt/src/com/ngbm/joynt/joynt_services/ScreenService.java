...
import com.ngbm.joynt.Engine;
import com.ngbm.joynt.HomeActivity;
import com.ngbm.joynt.IBaseScreen;
import com.ngbm.joynt.Joynt;
...
public class ScreenService extends NgnBaseService implements IScreenService {
	...
	public boolean back() {
		...
		if (mLastScreensIndex == 0) {
			if ((screen = mLastScreens[mLastScreens.length - 1]) == null) {
				// goto home
				return show(HomeActivity.class);
			} else {
	...
	public boolean bringToFront(int action, String[]... args) {
		Intent intent = new Intent(Joynt.getContext(), HomeActivity.class);
		try {
			...
			Joynt.getContext().startActivity(intent);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
	...
	public boolean show(Class<? extends Activity> cls, String id) {
		...
		final Window window = mainActivity.getLocalActivityManager()
				.startActivity(screen_id, intent);
		if (window != null) {
			View view = mainActivity.getLocalActivityManager()
					.startActivity(screen_id, intent).getDecorView();
	...
	public boolean show(String id) {
		final Activity screen = (Activity) ((Main) Engine.getInstance()
				.getMainActivity()).getLocalActivityManager().getActivity(id);
		if (screen == null) {
	...
	public boolean destroy(String id) {
		final LocalActivityManager activityManager = (((Main) Engine
				.getInstance().getMainActivity())).getLocalActivityManager();
	...
	public IBaseScreen getCurrentScreen() {
		return (IBaseScreen) ((Main) Engine.getInstance().getMainActivity())
	...
	public IBaseScreen getScreen(String id) {
		return (IBaseScreen) ((Main) Engine.getInstance().getMainActivity())
				.getLocalActivityManager().getActivity(id);
	}
}
