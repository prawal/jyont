package com.ngbm.joynt.json_helper;
...
public class UrlJsonAsyncTask extends AsyncTask<String, Void, JSONObject> {
	...
	private MySpinner progressDialog = null;
	...
	protected JSONObject doInBackground(String... urls) {
		return this.queryUrlForJson(urls[0], this.retryCount);
	}

	@Override
	protected void onPreExecute() {
		progressDialog = MySpinner.show(this.context, this.loadingTitle,
	...
	protected JSONObject queryUrlForJson(String url, int retries) {
		...
		try {
			try {
				...
				if (retries-- > 0) {
					json = queryUrlForJson(url, retries);
				} else {
